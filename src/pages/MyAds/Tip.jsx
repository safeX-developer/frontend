import React from 'react'

export default function Tip({handleCancel}) {

  return (
    <div>
        <div className="ant-modal-root css-5jb6ku">
            <div className="ant-modal-mask"></div>
            <div tabindex="-1" className="ant-modal-wrap ant-modal-centered">
                <div role="dialog" aria-labelledby=":rdo:" aria-modal="true" className="ant-modal css-5jb6ku css-5jb6ku ant-modal-custom bds-theme-component-light" style={{width: "480px",transformOrigin: "-138.333px 178.31px"}}>
                    <div tabindex="0" aria-hidden="true" style={{width: "0px", height: "0px", overflow: "hidden", outline: "none"}}></div>
                    <div className="ant-modal-content">
                        <button onClick={handleCancel} type="button" aria-label="Close" className="ant-modal-close">
                            <span className="ant-modal-close-x">
                                <svg width="1em" height="1em" viewBox="0 0 16 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M11.3002 12.243C11.5606 12.5034 11.9827 12.5034 12.243 12.243C12.5034 11.9827 12.5034 11.5606 12.243 11.3002L8.94327 8.00046L12.2431 4.70059C12.5035 4.44024 12.5035 4.01813 12.2431 3.75778C11.9828 3.49743 11.5607 3.49743 11.3003 3.75778L8.00046 7.05765L4.70057 3.75776C4.44022 3.49741 4.01811 3.49741 3.75776 3.75776C3.49741 4.01811 3.49741 4.44022 3.75776 4.70057L7.05765 8.00046L3.75786 11.3003C3.49751 11.5606 3.49751 11.9827 3.75786 12.2431C4.0182 12.5034 4.44031 12.5034 4.70066 12.2431L8.00046 8.94327L11.3002 12.243Z"></path>
                                </svg>
                            </span>
                        </button>
                    <div className="ant-modal-header">
                        <div className="ant-modal-title" id=":rdo:">Tips</div>
                    </div>
                    <div className="ant-modal-body">
                        <div className="ant-modal-custom-content" data-testid="lux-modal">
                            <div className="ant-modal-custom-children">
                                <section className="p2p_level--wrap">
                                    <p className="p2p_level--wrap-decribe">As a "New" advertiser, you can only post advertisements selling crypto. Progressing to a "Regular" advertiser would grant you the following privileges:</p>
                                    <div className="p2p_level--wrap-content">
                                        <section className="p2p_level--next p2p_level--cur">
                                            <div className="p2p_level--wrap-current">
                                                <p>Levels: New</p>
                                                <p>Benefits</p>
                                            </div>
                                            <p className="p2p_level--next-line p2p_level--next-lineCur"></p>
                                            <section className="p2p_level--wrap-content-next">
                                                <div className="p2p_level--wrap-next">
                                                    <p>Max. Coin Quantity per Single Ad</p>
                                                    <p>1000</p>
                                                </div>
                                                <div className="p2p_level--wrap-next">
                                                    <p>Ad Direction</p><p>Sell</p>
                                                </div>
                                                <div className="p2p_level--wrap-next">
                                                    <p>Max. Active Ads (Same Direction &amp; Pair)</p>
                                                    <p>1</p>
                                                </div>
                                            </section>
                                        </section>
                                    </div>
                                    <div className="p2p_level--main-img">
                                        <img src="data:image/png;base64," alt="" />
                                    </div>
                                    <div className="p2p_level--wrap-content">
                                        <section className="p2p_level--next">
                                            <div className="p2p_level--wrap-current">
                                                <p>Regular</p>
                                                <p>Benefits</p>
                                            </div>
                                            <p className="p2p_level--next-line"></p>
                                            <section className="p2p_level--wrap-content-next">
                                                <div className="p2p_level--wrap-next">
                                                    <p>Max. Coin Quantity per Single Ad</p>
                                                    <p>20000</p>
                                                </div>
                                                <div className="p2p_level--wrap-next">
                                                    <p>Ad Direction</p>
                                                    <p>Buy / Sell</p>
                                                </div>
                                                <div className="p2p_level--wrap-next">
                                                    <p>Max. Active Ads (Same Direction &amp; Pair)</p>
                                                    <p>2</p>
                                                </div>
                                            </section>
                                        </section>
                                    </div>
                                   
                                </section>
                            </div>
                            <div className="ant-modal-custom-footer">
                                <button type="button" className="ant-btn css-5jb6ku ant-btn-primary css-5jb6ku ant-btn-custom ant-btn-custom-middle ant-btn-custom-primary bds-theme-component-light">
                                    <span>OK</span>
                                </button>
                            </div>
                        </div>
                    </div>
                </div>
                <div tabindex="0" aria-hidden="true" style={{width: "0px", height: "0px", overflow: "hidden", outline: "none"}}></div>
            </div>
        </div>
    </div>
</div>
  )
}
